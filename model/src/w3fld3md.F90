...
Module W3FLD3MD
  ...
  !     ----------------------------------------------------------------
  !      W3FLD3    Subr. Public   Donelan et al. 2012 stress calculation
  !                               Compute z0 using 10m-wind speed only
  !                               when wind speed is greater than 40 m/s
  !     ----------------------------------------------------------------
  !
...
CONTAINS
  !/ ------------------------------------------------------------------- /
  SUBROUTINE W3FLD3(  ASPC,FPI, WNDX,WNDY, ZWND,                 &
    ...
    DO WHILE ( WIFLG .AND. NO_ERR )  !Wind iteration
      ...
      !         print*,'Attn: W3FLD3 Wind error gt 5%'
    ...
    IF (.not.((CD .LT. 0.01).AND.(CD .GT. 0.0005)).or. .not.(NO_ERR)) THEN
      !Fail safe to bulk
      print*,'Attn: W3FLD3 failed, using bulk stress'
